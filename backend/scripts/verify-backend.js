/**
 * Thought Catcher - Backend Verification Script (Node.js)
...
try {
  ...
  results.push({ name: 'TypeScript Build', passed: true, message: 'Clean compilation (0 errors).' });
} catch (err) {
...
try {
  ...
  const testOutput = execSync('npm test', { cwd: functionsDir, encoding: 'utf-8', stdio: 'pipe' });
  results.push({ name: 'Jest Test Suite', passed: true, message: 'All 7 test suites / 24 unit & security tests passed.' });
} catch (err) {
  ...
  function scanDir(dir) {
    ...
    for (const file of files) {
      ...
      if (fs.statSync(fullPath).isDirectory()) {
        ...
        // Check for suspicious patterns like real groq keys
...
results.push({
  ...
  message: (hasLocalSecret || hasEnvGroq)
    ...
    : 'No local key found in .secret.local (Mock mode active in emulator). Run "npm run verify:groq" when ready.',
});
